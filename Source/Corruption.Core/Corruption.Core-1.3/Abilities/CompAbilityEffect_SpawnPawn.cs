...
namespace Corruption.Core.Abilities
{
    public class CompAbilityEffect_SpawnPawn : CompAbilityEffect_WithDuration
    {
        ...
        public override void Apply(LocalTargetInfo target, LocalTargetInfo dest)
        {
            ...
            {
                Pawn newPawn = PawnGenerator.GeneratePawn(Props.kindDef, this.Props.isPsykerControlled ? this.parent.pawn.Faction : Find.FactionManager.FirstFactionOfDef(Props.kindDef.defaultFactionType)); // Find.FactionManager.FirstFactionOfDef(Corruption.Core.Abilities.FactionsDefOf.IoM_NPC));
                ...
                foreach (var hediffDef in this.Props.spawningHediffs)
                {
                    ...
                    HediffComp_Disappears hediffComp_Disappears = hediff.TryGetCompFast<HediffComp_Disappears>();
